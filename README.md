...
- **ターミナル**: iTerm2 設定
- **ノート管理**: nb + カスタム検索機能（nbq）
- **スニペット**: zeno によるコマンドスニペット管理
...
| オプション/環境変数   | 説明                                                                                                              |
| --------------------- | ----------------------------------------------------------------------------------------------------------------- |
| `--private`           | プライベート設定を有効化します。`Brewfile.private`、`plugins.private.toml`、nb/zeno のプライベート設定を読み込みます。 |
...
`config/zsh/zeno.zsh` で定義されているカスタム関数。nb（ノート管理ツール）の検索結果を fzf でプレビュー表示しながら選択できます。
